...
def crack(i: int, num: int, hashes: list, results: dict, change: Event, barrier: Barrier):
    ...
    if not len(hashes) == 0:
        barrier.wait()
        change.clear()
        crack(i, num, hashes, results, change, barrier)
...
def brute(list_of_hashes) -> list:
    ...
    with Manager() as manager:
        ...
        barrier = Barrier(processe_count)
        processes = []
        change = Event()
        for processe_num in range(processe_count):
            processe = Process(target=crack, args=(processe_num, processe_count, hashes, results, change, barrier))
            processes.append(processe)
            processe.start()
        for processe in processes:
            processe.join()    
        return [results[item] for item in list_of_hashes] # ensures the order of results matches the input order
